                                    ...
                                    with _btn_col2:
                                        ...
                                        for _pt_I, _pt_II in zip(points_I, points_II):
                                            ...
                                            if fringe_mode == "Mode I (G₁c)":
                                                _export_n[_eid] = _nI
                                            elif fringe_mode == "Mode II (G₂c)":
                                                _export_n[_eid] = _nII
                                            else:
                                                _valid = [v for v in [_nI, _nII] if not np.isnan(v) and v > 0]
                                                _export_n[_eid] = min(_valid) if _valid else np.nan
                                        ...
                                        # Map each element to its 0-based position in the keyfile.
                                        # SCLFringeDCToModel expects values in keyfile/database order,
                                        # so we index results[] directly — no runtime ID lookup needed.
                                        _all_eids_kf = (
                                            list(model._keyfile_data.elements.keys())
                                            if model._keyfile_data is not None else []
                                        )
                                        ...
                                        _scl = [
                                            ...
                                            f" * Mode      : {fringe_mode}",
                                            f" * Scale     : {_scale_label}",
                                            f" * Generated by LS-DYNA Post-Processor",
                                            f" * Values assigned by 0-based keyfile position index.",
                                            " */",
                                            "",
                                            "define:",
                                            "void main(void)",
                                            ...
                                            "    char buf[256];",
                                            "",
                                            '    numSolidEle = SCLGetDataCenterInt("num_solid_elements");',
                                            '    numStates   = SCLGetDataCenterInt("num_states");',
                                            "    results = malloc(numSolidEle * sizeof(Float));",
                                            "",
                                            "    /* Initialise – elements without data get 0 */",
                                            "    for(i = 0; i < numSolidEle; i = i+1) {",
                                            "        results[i] = 0.0;",
                                            "    }",
                                            "",
                                            "    /* Direct position assignments (keyfile order, 0-based) */",
                                        ] + _pos_lines + [
                                            "",
                                            f'    sprintf(buf, "{_cb_title}");',
                                            "",
                                            "    /* Apply the same fringe to every time state */",
                                            "    for(state = 1; state <= numStates; state = state+1) {",
                                        ...
                                        st.download_button(
                                            label="💾 Export Fringe (.scl)",
                                            data="\n".join(_scl),
                                            file_name=f"n_fringe_{selected_lcid}.scl",
                                            mime="text/plain",
                                            key="gn_fringe_export",
